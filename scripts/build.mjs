...
await writeFile(resolve(output, 'build-info.json'), `${JSON.stringify({ sourceCommit, sourceBranch, builtAt: new Date().toISOString() }, null, 2)}\n`);

const files = await readdir(output, { recursive: true });
const textFiles = files.filter((file) => ['.html', '.js', '.css', '.json'].includes(extname(file)));
const builtText = (await Promise.all(textFiles.map((file) => readFile(resolve(output, file), 'utf8')))).join('\n');

for (const requiredText of ['矿井排水危机', '御前问策', '/api/court', '大模型负责生成群臣意见', '颁布工程令', '规则审计', 'build-info.json', sourceCommit]) {
  if (!builtText.includes(requiredText)) throw new Error(`Production build contract failed: missing “${requiredText}”`);
}
